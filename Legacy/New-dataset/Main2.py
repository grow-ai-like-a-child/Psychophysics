    ...
    for A in letter_colors:
        for B in letter_colors:
            ...
            for col_idx, word in enumerate(seq, start=1):
                ...
                # Stroop type1：第一列和第三列使用 color_map[A]，第二列使用 color_map[B]
...
def generate_stroop_letter_type2():
    ...
    for A in letter_colors:
        for B in letter_colors:
            ...
            for col_idx, word in enumerate(seq, start=1):
                cell = ws.cell(row=row_idx, column=col_idx, value=word)
                # Stroop type2：第一列和第三列使用 color_map[B]，第二列使用 color_map[A]
                if col_idx == 1 or col_idx == 3:
                    font_color = color_map[B]
                else:
                    font_color = color_map[A]
...
def generate_stroop_letter_type3():
    ...
    for A in letter_colors:
        for B in letter_colors:
            ...
            for col_idx, word in enumerate(seq, start=1):
                cell = ws.cell(row=row_idx, column=col_idx, value=word)
                # Stroop type3：第一列和第二列使用 color_map[A]，第三列使用 color_map[B]
...
def generate_stroop_letter_type4():
    ...
    for A in letter_colors:
        for B in letter_colors:
            ...
            for col_idx, word in enumerate(seq, start=1):
                cell = ws.cell(row=row_idx, column=col_idx, value=word)
                # Stroop type4：第一列和第二列使用 color_map[B]，第三列使用 color_map[A]
                if col_idx == 1 or col_idx == 2:
                    font_color = color_map[B]
                else:
                    font_color = color_map[A]
...
def save_to_excel(rows, task, style, dataset_type):
    """
    将生成的数据保存到 Excel 文件中，
    保存到程序同目录下的 Data-set/Flanker-<Style>/，
    Excel 文件名格式：<task>_<style>_<dataset_type>_dataset.xlsx
    ...
    os.makedirs(subfolder, exist_ok=True)
    filename = f"{task}_{style}_{dataset_type}_dataset.xlsx"
    filepath = os.path.join(subfolder, filename)
    # 对数据进行排序，使得 Title 混合显示
    header = rows[0]
    data = rows[1:]
    data_sorted = sorted(data, key=lambda x: x[0])
    rows_sorted = [header] + data_sorted
    df = pd.DataFrame(rows_sorted[1:], columns=rows_sorted[0])
    df.to_excel(filepath, index=False)
    print(f"数据已保存到 Excel 文件：{filepath}")
    return filepath

# ================= 生成图像 ===================
def generate_flanker_images_from_excel(excel_file, task, style, dataset_type):
    """
    根据 Excel 文件生成 Flanker 图片，
    图片保存在程序同目录下的 Data-set-Image/<Task>-<Style>-image/<dataset_type>/。
    使用 pandas 读取Excel，假定列名为：Title, Wrong_Option, Right_Option，
    文本位置：
      Title 显示在中上 (0.5, 0.70)；
      Wrong_Option 显示在中下偏左 (0.25, 0.45)；
      Right_Option 显示在中下偏右 (0.75, 0.45)。
    图片背景色为灰色，尺寸500x300像素（figsize=(5,3)，dpi=100）。
    文件名以数字序号（从1开始）开头，自动生成所有数据行的图片，不再询问行号。
    """
    ...
    for idx, row in df.iterrows():
        ...
        plt.text(0.25, 0.45, text_wrong, ha='center', va='center', fontsize=25, color='white')
        plt.text(0.75, 0.45, text_right, ha='center', va='center', fontsize=25, color='white')
        ...
        plt.close()
        count += 1
    print(f"Flanker 图片生成中... 共计 {count - 1} 张。")
...
def generate_stroop_images_from_excel(excel_file, task, style, dataset_type):
    ...
    图片保存在程序同目录下的 Data-set-Image/<Task>-<Style>-image/<dataset_type>/。
    使用 openpyxl 读取Excel（保留字体颜色信息），
    假定表头在第1行，列名为：Title, Wrong_Option, Right_Option，
    文本位置：
      Title 显示在中上 (0.5,0.70, fontsize=30)；
      Wrong_Option 显示在右下 (0.75,0.40, fontsize=25)；
      Right_Option 显示在左下 (0.25,0.40, fontsize=25)。
    背景色使用归一化灰色： (200/255,200/255,200/255)。
    ...
    for r in range(1, ws.max_row + 1):
        ...
        plt.close()
        count += 1
    print(f"Stroop 图片生成中... 共计 {count - 1} 张。")
...
def main():
    ...
    # 对于 flanker，继续询问数据类型（letter 或 number）；对于 stroop，直接使用 letter 版本
    if task == "flanker":
    ...
    if task == "flanker":
        if style == "letter":
            ...
            if dataset_type == "type1":
                ...
                print("未知 flanker number 类型。")
                return
            excel_filepath = save_to_excel(rows, task, style, dataset_type)
    elif task == "stroop":
        # stroop 不再询问 letter/number，直接生成 stroop letter 数据集
        if dataset_type == "type1":
